...
| `sleep` | `{ "seconds": 0.5 }` where `seconds` is a number from `0` through `5.0`. | `{ "slept_seconds": 0.5 }` | Uses bounded `asyncio.sleep` intervals; no subprocesses or host operations. Running workers can observe cancellation between intervals and stop cooperatively. |
...
| `fail_once` | `{}` | First attempt raises a safe handler error; later attempts return `{ "failed_once": true, "attempt": 2 }`. | Demonstrates retry without external side effects. |
| `always_fail` | `{}` | Always raises a safe handler error. | Demonstrates dead-letter handling without external side effects. |
